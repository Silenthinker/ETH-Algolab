...
void light1() {
    // Dynamic programming
    /* Numerate bulb from 1 to n.
     * dp[i][1/0] -> number of operations to sovle the 1...ith bulbs if flipped or not
     * i \in [0,n]
     */
    cin >> n >> k >> x;
    rep(i,n)
    ...
    for(int i=0; i<n+1; i+=k) {
        rep(j,2) {
            if (i==0) {
                dp[i][j] = 0; // no operation if no bulb
            }
            else {
                int err_cnt = 0;
                rep(m,k) {
                    int bulbm = BULBS[i-m-1];
                    if (j==1) bulbm = (bulbm+1)%2;
                    if (PATTERN[m]!=bulbm) err_cnt++;
                }
                dp[i][j] = min(err_cnt+dp[i-k][j], k-err_cnt+1+dp[i-k][(j+1)%2]);
            }
        }
    }
    cout<<dp[n][0]<<endl;
